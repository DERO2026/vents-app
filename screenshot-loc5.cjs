...
(async()=>{
  ...
  await waitFor(page,"Lagos Nigeria",8000)||await waitFor(page,"Open in Maps",6000)||await wait(4000);
  await wait(1000);
  // Scroll to bottom to see location message
  ...
  await wait(1500); await ss(page,"LOC-01-location-msg");
  const d=await body(page); process.stdout.write("body:"+d.replace(/\n/g," ").slice(-600)+"\n");
