...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // check is even reached. A legitimate extract-events request never sets
  // `mode`, and the AI-assistant client (src/lib/ventsAi.ts) always sets it,
  // so neither request shape can ever fall through into the other's branch
  // by accident.
  //
  // Folded into this existing endpoint (its actual handler logic lives
  // untouched in api/_lib/aiAssistantHandler.ts, which owns its own
  // auth/rate-limit/CORS) purely to stay within Vercel Hobby's
  // 12-serverless-function cap -- this file already hosts the vision-crop
  // branch below for the same reason. This routing is a pure dispatch -- no
  // extraction logic runs on this path, and no AI-assistant logic runs on
