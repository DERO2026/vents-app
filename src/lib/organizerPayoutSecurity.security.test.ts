...
// Regression tests for two critical defects found during a financial/
// accounting audit of the organizer/provider earnings withdrawal path
// (0076_fix_withdrawal_rls_hole_and_fail_payout_bug.sql). Both were
// confirmed against real, live Production function/policy definitions --
// not hypothesized -- and the RLS defect was confirmed exploitable using a
// real, non-superuser Postgres role with RLS actually enforced (not a
// mocked or superuser-bypassed check).
...
beforeAll(() => {
  ...
  m0076 = readFileSync(join(dir, '0076_fix_withdrawal_rls_hole_and_fail_payout_bug.sql'), 'utf8');
