-- CRITICAL FIX: establishes ONE authoritative definition of "has this
-- event ended" and applies it to every server-side purchase guard, plus
-- adds the 7-day archival sweep. See the accompanying report for the full
-- audit (client-side discovery/CTA fixes are in the same commit's TS
-- changes, not this file).
--
-- ROOT CAUSE: no single source of truth existed. Every path implemented
-- its own ad hoc, mostly-wrong notion of "ended":
--   - select_events RLS (0008): filters deleted_at only, no date predicate
--     at all -- by design, since ended events must stay visible to their
--     ticket-holders/organizer/admin (this migration does NOT touch it).
--   - finalize_pending_purchase / purchase_ticket (the actual purchase
--     RPCs -- the money path): `v_event.event_date::date < current_date`
--     -- compares only the DATE PART of the START timestamp, ignores
--     end_date entirely, and lets an event that started and already
--     ENDED earlier TODAY stay purchasable until midnight. This is the
--     literal root cause of "an event that ended weeks ago still shows a
--     Pay button" if that Pay tap ever reaches the server (client-side had
--     no gating at all -- see the TS changes).
--   - get_organizer_events_overview: `is_ended := event_date < now()` --
--     conceptually wrong (treats the START as the END), organizer-facing
--     display only, not a purchase gate, but still misleading.
--   - Trending/search scoring RPCs: `event_date >= now()` -- the opposite
--     problem (excludes an event that already started but is still
--     ongoing and purchasable). Not touched in this pass -- lower priority
--     than the over-inclusion bug this migration fixes; flagged in the
--     report as a follow-up.
--
-- FIX: one canonical SQL function defining "effective end", and one
-- canonical predicate built on it, used everywhere an authoritative
-- purchase/lifecycle decision is made server-side.
--
-- Effective-end rule: COALESCE(end_date, event_date + 24 hours).
-- end_date (timestamptz, already set for explicit multi-day events) is
-- authoritative when present. For a normal single-day event (end_date
-- NULL), start_time/end_time are free-text display fields (e.g. "7:00 PM")
-- with no guaranteed parseable format -- deriving an exact end from them
-- would reintroduce the same fragile string-parsing failure mode already
-- seen elsewhere in this codebase (MyTicketsScreen's own date-string
-- parsing). A flat 24h window from the start is a simple, conservative,
-- purely-timestamptz (no string parsing, no timezone ambiguity -- both
-- columns are already stored in UTC and compared directly against now())
-- rule that covers every realistic single-day event without cutting one
-- off mid-event, while still correctly retiring an event within a day of
-- it starting rather than leaving it purchasable indefinitely.
CREATE OR REPLACE FUNCTION public.event_effective_end_at(p_event_date timestamptz, p_end_date timestamptz)
 RETURNS timestamptz
 LANGUAGE sql
 IMMUTABLE
AS $function$
  SELECT COALESCE(p_end_date, p_event_date + interval '24 hours');
$function$
;

REVOKE ALL ON FUNCTION public.event_effective_end_at(timestamptz, timestamptz) FROM PUBLIC, anon, authenticated, project_admin;
GRANT EXECUTE ON FUNCTION public.event_effective_end_at(timestamptz, timestamptz) TO anon, authenticated, project_admin;

-- The single canonical "is this event currently purchasable/discoverable"
-- predicate -- every other status/visibility flag (deleted, hidden,
-- status) folded in alongside the time check, so callers never have to
-- separately remember all four conditions themselves.
CREATE OR REPLACE FUNCTION public.event_is_active(
  p_event_date timestamptz,
  p_end_date timestamptz,
  p_status text,
  p_hidden_by_admin boolean,
...
AS $function$
  SELECT p_deleted_at IS NULL
     AND p_hidden_by_admin IS NOT TRUE
     AND p_status IN ('live', 'published')
     AND now() < public.event_effective_end_at(p_event_date, p_end_date);
$function$
;

REVOKE ALL ON FUNCTION public.event_is_active(timestamptz, timestamptz, text, boolean, timestamptz) FROM PUBLIC, anon, authenticated, project_admin;
GRANT EXECUTE ON FUNCTION public.event_is_active(timestamptz, timestamptz, text, boolean, timestamptz) TO anon, authenticated, project_admin;

-- ── finalize_pending_purchase: server-side expiry guard (the real fix for
-- requirement #10 -- a stale client/direct request can no longer buy a
-- ticket to an ended event even if it somehow reaches this far). Full
-- function body otherwise UNCHANGED from its prior definition -- only the
-- SELECT's column list (added end_date) and the expiry IF differ. ──────
...
-- ── purchase_ticket: same server-side expiry guard fix as
-- finalize_pending_purchase above (this is the free/VC-discounted-ticket
-- path; purchase_ticket_with_tokens is a thin wrapper around this function
-- and needs no separate change). Full body otherwise UNCHANGED -- only the
-- SELECT's column list (added end_date) and the expiry IF differ. ──────
...
-- ── 7-day archival sweep (requirement #7) ───────────────────────────────
-- A soft marker column, NOT a hard delete: tickets, payments, check-ins,
-- transfers, and analytics all reference events by id (requirement #8), so
-- deleting the row would either cascade-destroy or orphan those audit/
-- transactional records. archived_at just tells customer-facing queries to
-- stop surfacing the event; ticket-holders' My Tickets -> Past and
-- organizer/admin management views are keyed off tickets/organizer_id/
-- is_admin(), not off this flag, so they are unaffected (requirements #5,
-- #6).
ALTER TABLE public.events ADD COLUMN IF NOT EXISTS archived_at timestamptz;

-- Idempotent by construction: only touches rows that are still unarchived
-- AND whose effective end passed more than 7 days ago, and only ever sets
-- archived_at once (COALESCE-free re-runs are simply no-ops once a row is
-- archived). Invoked from the EXISTING daily cron (api/cron/run.ts) --
-- deliberately NOT a new/hourly cron (requirement #11; Vercel Hobby caps
-- cron frequency at daily regardless of configured schedule).
...
BEGIN
  ...
  WHERE archived_at IS NULL
    AND now() >= public.event_effective_end_at(event_date, end_date) + interval '7 days';
...
GRANT EXECUTE ON FUNCTION public.archive_ended_events() TO project_admin;
