...
async function main() {
  ...
  const email = `test-upd-login-${randomSuffix}@example.com`;
  const password = "password123";
  const name = `Test Update Login ${randomSuffix}`;
  ...
  console.log("SignIn succeeded. User ID:", userId);

  // Authenticate client
  insforge.auth.setAccessToken(signinRes.data.accessToken);
