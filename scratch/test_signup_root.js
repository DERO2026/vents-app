...
async function main() {
  ...
  const email = `test-root-${randomSuffix}@example.com`;
  const password = "password123";
  const name = `Test Root ${randomSuffix}`;
  const role = "organizer";
  const username = `testroot${randomSuffix}`;
  const phone = `+234803${String(randomSuffix).padStart(6, '0')}`;
  const state = "Lagos";

  console.log("--- 1. Testing signUp with root options ---");
