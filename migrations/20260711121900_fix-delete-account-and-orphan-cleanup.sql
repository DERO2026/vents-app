-- Fix delete_own_account() referencing the nonexistent 'event_saves' table
-- (the real table is 'saved_events') and add cleanup_orphaned_records() for
-- the admin Systems tab's "Orphaned Record Cleanup" utility.
...
-- Admin-only: soft-delete tickets and remove saved_events rows that point at
-- events which no longer exist. Both tables already have ON DELETE CASCADE
-- FKs to events.id, so true orphans should be rare/historical-only; this is
-- a defensive one-off/periodic sweep, not a fix for an actively-recurring bug.
