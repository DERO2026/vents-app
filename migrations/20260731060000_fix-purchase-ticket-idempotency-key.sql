-- purchase_ticket's "idempotency" short-circuit checked
-- (event_id, user_id, status='active') — ANY prior active ticket for this
-- event, not the SPECIFIC payment_ref being retried. Once a user held one
-- active ticket for an event, every later purchase call for that same
-- event (a genuinely new purchase — different ticket type, quantity, or
-- Paystack reference) silently short-circuited to the OLD ticket ids and
-- inserted nothing new. Paystack charges the new payment_ref; the buyer
-- receives zero new tickets for it. Confirmed live in production via
-- pg_proc before this fix.
--
-- Fix: only short-circuit when a ticket already exists for THIS EXACT
-- payment_ref (a true retry of the same purchase attempt) — a new
-- payment_ref for the same event/user now correctly proceeds to purchase.
