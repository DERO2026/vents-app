...
-- (20260801002441_escape-wildcards-in-fuzzy-search.sql), which carried
-- forward a pre-existing, currently-live production bug unchanged from the
-- prior definition: `SELECT date_of_birth INTO v_dob FROM public.users
-- WHERE id = auth.uid()` is ambiguous, because this function's RETURNS
-- TABLE declares an output column also named `id`, which PL/pgSQL treats
-- as an in-scope variable for the whole function body. The anon key's own
-- JWT carries a placeholder `sub` claim, so auth.uid() is non-null for
-- virtually every caller — not just logged-in users — meaning this branch,
-- and this exact error, fired for nearly every search request. Live-
-- confirmed before this fix: calling search_events_fuzzy over the REST RPC
-- with the anon key returned 42702 "column reference \"id\" is ambiguous".
-- Qualifies the column reference to resolve it.
