...
(async()=>{
  ...
  if(!isLoggedIn){
    ...
    if(bl.includes("get started")||bl.includes("welcome")){
      await page.evaluate(()=>{const el=[...document.querySelectorAll("*")].find(e=>e.textContent.includes("Sign in")&&!e.children.length);if(el)el.click();});await wait(2000);
    }
    ...
    await page.evaluate((em,pw)=>{
      ...
      fill(document.querySelector('input[type="email"],input[type="text"]'),em);
  ...
  await wait(2000);
  await ss(page,"LOC-02-scrolled-bottom");
